...
# Version: 1.0.3 (GTK3, GPU, Drag & Drop)
...
class VideoConverterWindow(Gtk.Window):
    def __init__(self):
        ...
        # Qualität
        grid.attach(Gtk.Label(label="Qualität:", xalign=0), 0, 3, 1, 1)
        ...
        grid.attach(Gtk.Label(label="Wert:", xalign=0), 0, 4, 1, 1)
        ...
        # Start/Stop Buttons unten links
        ...
        # --- Rechte Seite: Liste, Progress, Log ---
        ...
        column = Gtk.TreeViewColumn("Ausgewählte Dateien (Drag & Drop möglich)", renderer, text=0)
        ...
        # Log
        right_vbox.pack_start(Gtk.Label(label="ffmpeg-Ausgabe (live):", xalign=0), False, False, 0)
        ...
        self.show_all()
        self.append_log("GuideOS-Videokonverter bereit.\n")
    ...
    def on_quality_mode_changed(self, combo):
        ...
        if "CQ" in mode: self.quality_entry.set_text("23")
        elif "Bitrate" in mode: self.quality_entry.set_text("5000")
        else: self.quality_entry.set_text("700")

    def on_select_files(self, btn):
        dialog = Gtk.FileChooserDialog(
            title="Wähle Videodateien", parent=self,
            action=Gtk.FileChooserAction.OPEN,
            buttons=(Gtk.STOCK_CANCEL, Gtk.ResponseType.CANCEL, Gtk.STOCK_OPEN, Gtk.ResponseType.OK)
        )
        dialog.set_select_multiple(True)
        filter_vid = Gtk.FileFilter()
        filter_vid.set_name("Videodateien")
        for ext in ["*.mp4","*.mkv","*.mov","*.avi","*.webm"]: filter_vid.add_pattern(ext)
    ...
    def on_browse_target(self, btn):
        dialog = Gtk.FileChooserDialog(
            title="Zielverzeichnis", parent=self,
    ...
    def on_drag_data_received(self, widget, context, x, y, selection, info, time):
        ...
        for uri in uris:
            ...
            p = Path(path)
            if p.exists() and p.suffix.lower() in [".mp4",".mkv",".mov",".avi",".webm"]:
                if path not in self.selected_files:
                    self.selected_files.append(path)
                    self.liststore.append([p.name])
    ...
    def build_ffmpeg_args(self, infile, outfile):
        ...
        if vchoice == "Nur Audio ändern":
            ...
            if "CQ" in qmode:
                ...
                if any(x in codec for x in ["nvenc", "amf", "vaapi"]):
                ...
                args += ["-c:v", codec, "-b:v", f"{vkbps}k", "-preset", "p4"]
    ...
    def run_conversion(self):
        ...
        for idx, infile in enumerate(list(self.selected_files), 1):
            ...
            duration = probe_duration_seconds(in_p) or 1.0

            args = self.build_ffmpeg_args(str(in_p), str(out_p)) + ["-y", str(out_p)]
            self.append_log(f"Datei {idx}/{total}: {in_p.name}\n")
