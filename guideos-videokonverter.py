...
# Version:  1.1.3 (GNOME HeaderBar Title Logo Integration)
...
class VideoConverterWindow(Gtk.Window):
    def __init__(self):
        ...
        # --- VARIANTE C: Moderne HeaderBar mit integriertem Logo ---
        ...
        if logo_file.exists():
            try:
                # Setzt Fenster- & Taskleisten-Icon
                #self.set_icon_from_file(str(logo_file))

                # Fügt das Logo links in die HeaderBar ein
                pixbuf = Pixbuf.new_from_file_at_scale(str(logo_file), width=28, height=28, preserve_aspect_ratio=True)
                header_logo = Gtk.Image.new_from_pixbuf(pixbuf)
                header_logo.set_margin_start(4)
                header_logo.set_margin_end(4)
                header_bar.pack_start(header_logo)
            except Exception as e:
                print(f"Konnte HeaderBar-Logo nicht laden: {e}")
    ...
    def build_ffmpeg_args(self, infile, outfile):
        ...
        dur_limit = str(sanitize_int(self.duration_limit_entry.get_text(), default=0))
        if dur_limit != "0":
            args += ["-t", dur_limit]
